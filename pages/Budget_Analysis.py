...
def clean_currency(value):
    # Converts various formats (like strings with commas) to a numeric type
...
st.markdown("This section highlights changes made to the DPWH budget during the legislative process.")
try:
    ...
    # The first column name is messy, so we rename them all for reliability
...
try:
    df_nga = pd.read_excel(
        "data/NGAs Budget per FY.xlsx",
        sheet_name="TOTAL GAA per Agency",
        skiprows=5 # Skip the first 5 rows of titles to get to the data
    )
    ...
    st.error(f"Could not load national agency data from 'NGAs Budget per FY.xlsx'. Error: {e}")
